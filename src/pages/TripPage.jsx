        ...
        {editing ? (
          <form onSubmit={saveEdit} className="flex items-start gap-3">
            <EmojiPicker
              ...
              onChange={e => setEditForm(f => ({ ...f, cover_emoji: e }))}
            />
            <div className="flex-1 space-y-2">
              ...
              <div className="flex gap-2">
                <button type="submit" className="flex items-center gap-1 px-3 py-1.5 bg-stone-800 text-white rounded-lg text-xs font-medium hover:bg-stone-700">
                  <Check size={13} /> Save
                </button>
                <button type="button" onClick={cancelEdit} className="flex items-center gap-1 px-3 py-1.5 border border-stone-300 text-stone-600 rounded-lg text-xs hover:bg-stone-50">
                  <X size={13} /> Cancel
                </button>
              </div>
            </div>
          ...
          <div className="flex items-start gap-3 group">
              ...
              {(() => {
                ...
                return d ? (
                  <p className="flex items-center gap-1.5 text-xs text-stone-400 mt-0.5">
                    <Calendar size={12} />
                    {d.dateStr}
                    {d.duration && <span className="text-stone-300">·</span>}
                    {d.duration && <span>{d.duration} day{d.duration !== 1 ? 's' : ''}</span>}
                  </p>
                ) : null
              })()}
            </div>
            <button
              onClick={startEdit}
              className="p-1.5 text-stone-300 hover:text-stone-600 rounded-lg hover:bg-stone-100 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5"
              title="Edit trip"
            >
              <Pencil size={15} />
            </button>
          ...
          {['itinerary', 'diary'].map(t => (
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-sm font-medium capitalize transition-colors ${
                tab === t ? 'bg-stone-800 text-white' : 'text-stone-500 hover:text-stone-800 hover:bg-stone-100'
              }`}
            >{t}</button>
          ))}
        </div>
